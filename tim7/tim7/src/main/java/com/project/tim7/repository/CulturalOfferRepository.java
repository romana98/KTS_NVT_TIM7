...
public interface CulturalOfferRepository extends JpaRepository<CulturalOffer, Integer> {
    ...
    @Query(value = "SELECT * from cultural_offers where location_id in (SELECT id from locations where upper(name) LIKE upper(CONCAT('%', :value, '%')))", nativeQuery = true)
    Page<CulturalOffer> filterByLocation(String value, Pageable pageable);

    @Query(value = "SELECT * from cultural_offers where upper(name) LIKE upper(CONCAT('%', :value, '%'))", nativeQuery = true)
    Page<CulturalOffer> filterByName(String value, Pageable pageable);

	@Query(value = "SELECT * from cultural_offers where upper(name) LIKE upper(CONCAT('%', :value, '%')) or location_id in (SELECT id from locations where upper(name) LIKE upper(CONCAT('%', :value, '%'))) or subcategory_id in (SELECT id from subcategories where upper(name) LIKE upper(CONCAT('%', :value, '%'))) or subcategory_id in (SELECT id from subcategories where category_id in (SELECT id from categories where upper(name) like upper(CONCAT('%', :value, '%'))))", nativeQuery = true)
